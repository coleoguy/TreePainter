...
dat <- as.data.frame(matrix(data = NA,
                            nrow = length(nTrees) * length(rateClasses) * length(stepSize) * length(scaler),
                            ncol = 8))
# give column names
colnames(dat) <- c("tree", "rep", "Ntips", "rateClasses", "stepSize", "scaler", "truePositives", "falsePositives")

results <- vector(mode = "list", length = reps)

# make a counter
counter <- 1
# run the analysis
x <- foreach(j = 1:reps, .verbose = T, .packages = c("diversitree", "castor"),.options.snow=opts) %dopar% {
  counter <- 1
  print(paste("working on iteration", j))
